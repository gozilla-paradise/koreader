...
bool ldomXRange::getWordRange( ldomXRange & range, ldomXPointer & p )
{
    ...
    if ( pos>(int)txt.length() )
        pos = txt.length();
    // Thai-aware word selection: if the caret is inside a Thai run, use the
    // trie-based segmenter to find exact word boundaries instead of the
    // generic space-scan below.
