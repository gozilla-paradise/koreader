...
#define ITEMS_RESERVED 16
    ...
    void copyText( int start, int end )
    {
        ...
        #if (USE_LIBUNIBREAK==1)
        // Thai line-break post-pass: override libunibreak's no-break decisions
        // for Thai runs using the trie-based segmenter.  The pass runs after
        // bidi processing so all indices into m_text / m_flags are final.
        if (ThaiSegmenter::isEnabled() && ThaiSegmenter::containsThai(m_text, m_length)) {
            int ti = 0;
            while (ti < m_length) {
                if (m_text[ti] >= 0x0E00 && m_text[ti] <= 0x0E7F) {
                    int run_start = ti;
                    while (ti < m_length && m_text[ti] >= 0x0E00 && m_text[ti] <= 0x0E7F) ti++;
                    int run_len = ti - run_start;
                    if (run_len >= 2) {
